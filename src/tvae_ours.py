...
class TVAE(BaseSynthesizer):
    ...
    def fit(self, train_data, discrete_columns=()):
        ...
        print(sum(p.numel() for p in self.decoder.parameters()))
    ...
    def encode(self, data):
        ...
        data = torch.from_numpy(data.astype('float32')).to(self._device)
        # dataset = TensorDataset(torch.from_numpy(data.astype('float32')).to(self._device))
        mu, std, logvar = self.encoder(data)
        return mu.detach().cpu().numpy(), std.detach().cpu().numpy(), logvar.detach().cpu().numpy()
